...
// Release releases the lock that was created when creating this reader.
// Successfully calling this function more than once will result in no-op.
...
// CloseAndRelease closes the reader and releases the lock AFTER wg.Done() was
// called.
//
// IMPORTANT: You should call wg.Done() when the channel is closed and consumed.
// Failing to call wg.Done() will result in this call hanging indefinitely.
...
// CloseUpdateCheckpointAndRelease closes the reader and updates the checkpoint
// of the reader and releases the lock AFTER wg.Done() was called.
//
// IMPORTANT: You should call wg.Done() when the channel is closed and consumed.
// Failing to call wg.Done() will result in this call hanging indefinitely.
