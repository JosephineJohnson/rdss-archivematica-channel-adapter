...
// NewReader initialize a reader on a shard with default parameters. It reads a
// batch of 100 records from a shard every 100 ms.
...
// NewReaderWithParameters initialize a reader on a shard defining how often a
// shard should be read, the size of a read batch and the channel buffer.
...
// Records consumes a shard from the last checkpoint if any, otherwise starts at
// the last untrimmed record. It returns a read-only buffered channel via which
// results are delivered.
//
// NOTE: checkpoints are NOT automatically set, you have to do it via
// UpdateCheckpoint function ideally after you call Close and consume all
// messages from the channel. If you want to make checkpoints while consuming
// the stream and be 100% safe that no messages got unprocessed you should call
// BlockReading, consume the channel, call UpdateCheckpoint and then call
// ResumeReading.
...
// UpdateCheckpoint sets the checkpoint to the last record that was read. It
// waits for the current batch to be processed and pushed to the channel.
func (r *Reader) UpdateCheckpoint() error {
	// Acquire lock so that checkpoint increments don't get discarded while
	// updating checkpoint
...
// BlockReading stops reading from the stream after the current batch is
// processed. This could be used to safely update checkpoints before the reader
// is closed.
...
// Close waits for the current batch to be read and pushed to the channel and
// then stops the reading and closes the channel. No further records will be
// read from the stream. After calling close and consuming the channel is safe
// to call UpdateCheckpoint.
